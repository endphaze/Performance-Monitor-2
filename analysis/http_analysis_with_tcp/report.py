...
from .graph_generator3 import plot_graph_rps, plot_graph_response_time_threshold
from utility.outputmodel import BaseOutputModel

import pandas as pd
import os

def generate_report(result : BaseOutputModel):
    ...
    output_pdf = f"{csv_path}/report_tcp_{target_ip}.pdf"
    
    base_name = os.path.splitext(os.path.basename(csv_file))[0]
    
    # check ว่า header เป็น pattern ไหน
    if header_list == ['number', 'time', 'payload_len', 'response_time', 'pending_req', 'stream_id', 'type']:
        # plot graph ก่อนและกำหนด path ของ report
        rps_graph = plot_graph_rps(df, base_name, csv_path)     
        rt_graph = plot_graph_response_time_threshold(df, base_name, csv_path)
