...
def plot_graph_rps(df, base_name, result_path):
    # 1. โหลดข้อมูล
    ...
    save_path = f'{result_path}/graph/{base_name}_split_graph.png'
...
def plot_graph_response_time_threshold(df, base_name, result_path, thresholds=None):
    """
    สร้างกราฟ Response Time พร้อมระบบ Threshold แจ้งเตือน
    :param thresholds: dict เช่น {500: 'orange', 1000: 'red'}
    """
    # 1. เตรียมข้อมูล
    df['dt'] = pd.to_datetime(df['time'], unit='s')
    
    # 2. กรองข้อมูลเฉพาะ 'response'
    df_res = df[df['type'] == 'response'].copy()
    if df_res.empty:
        print("ไม่พบข้อมูลประเภท 'response' ในชุดข้อมูล")
        return None
    df_res.set_index('dt', inplace=True)
    
    # 3. คำนวณค่าเฉลี่ยรายวินาที
    res_avg_sec = df_res['response_time'].resample('1s').mean().fillna(0)
    ...
    # วาดเส้นหลัก
    ax.plot(res_avg_sec.index, res_avg_sec.values, color='forestgreen', 
            linewidth=2, label='Avg Response Time', alpha=0.7)
    
    # 5. เพิ่มระบบ Threshold และจุดแจ้งเตือน
    if thresholds:
        # เรียงลำดับจากค่าน้อยไปมากเพื่อให้จุดสีทับกันถูกต้อง
        sorted_thresholds = sorted(thresholds.items())
        for level, color in sorted_thresholds:
            # หาจุดที่ค่าเกิน threshold
            over_threshold = res_avg_sec[res_avg_sec >= level]
            if not over_threshold.empty:
                ax.scatter(over_threshold.index, over_threshold.values, 
                           color=color, s=30, label=f'Alert >= {level} ms', zorder=5)
                # วาดเส้นประแนวนอน
                ax.axhline(y=level, color=color, linestyle='--', alpha=0.4, linewidth=1)
    ...
    # 6. บันทึกรูปภาพ
    os.makedirs(f'{result_path}/graph', exist_ok=True)
    save_path = f'{result_path}/graph/{base_name}_response_time.png'
    plt.savefig(save_path, dpi=300)
    plt.close() # ปิดเพื่อคืนค่า memory
    
    print(f"บันทึกกราฟ Response Time พร้อม Threshold เรียบร้อยที่: {save_path}")
    return save_path
